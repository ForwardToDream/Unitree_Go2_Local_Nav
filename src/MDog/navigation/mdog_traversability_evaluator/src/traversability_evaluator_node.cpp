...
class MDogTraversabilityEvaluatorNode : public rclcpp::Node {
 public:
  MDogTraversabilityEvaluatorNode() : Node("mdog_traversability_evaluator") {
    ...
    grid_sub_ = create_subscription<mdog_grid_msgs::msg::LocalGrid>(
        ...
        [this](mdog_grid_msgs::msg::LocalGrid::SharedPtr msg) { processGrid(*msg); });
    trav_pub_ = create_publisher<mdog_traversability_msgs::msg::Traversability>(output_topic_, 10);

    RCLCPP_INFO(get_logger(), "Traversability evaluator: %s -> %s",
                local_grid_topic_.c_str(), output_topic_.c_str());
  }

 private:
  void processGrid(const mdog_grid_msgs::msg::LocalGrid& grid) {
    ...
    mdog_traversability_msgs::msg::Traversability msg;
  ...
  CorridorResult evaluateCorridor(
      const mdog_grid_msgs::msg::LocalGrid& grid, double heading_rad, double distance_m) const {
  ...
  bool isPassableLabel(uint8_t label) const {
    return label == mdog_grid_msgs::msg::GridCell::FREE ||
           label == mdog_grid_msgs::msg::GridCell::UNKNOWN;
  }
  ...
  rclcpp::Publisher<mdog_traversability_msgs::msg::Traversability>::SharedPtr trav_pub_;
};

int main(int argc, char** argv) {
  rclcpp::init(argc, argv);
  rclcpp::spin(std::make_shared<MDogTraversabilityEvaluatorNode>());
  rclcpp::shutdown();
  return 0;
}
