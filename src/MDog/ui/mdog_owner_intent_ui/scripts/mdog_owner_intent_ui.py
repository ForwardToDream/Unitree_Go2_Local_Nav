...
from mdog_intent_msgs.msg import OwnerIntent
from mdog_planning_msgs.msg import NavDecision, NavFeedback
...
class MDogOwnerIntentUi(Node):
    def __init__(self):
        ...
        self.nav_feedback_topic = self.get_parameter("nav_feedback_topic").value
        self.nav_decision_topic = self.get_parameter("nav_decision_topic").value
        self.publish_rate = max(1.0, float(self.get_parameter("publish_rate").value))
        self.default_speed = max(0.0, float(self.get_parameter("default_speed").value))

        self.publisher = self.create_publisher(OwnerIntent, self.intent_topic, 10)
        self.cmd_vel_sub = self.create_subscription(Twist, self.cmd_vel_topic, self.on_cmd_vel, 10)
    ...
    def _build_ui(self):
        ...
        ttk.Label(header, text="MDog Owner Intent", font=("", 16, "bold")).grid(row=0, column=0, sticky="w")
        ttk.Label(header, text=f"topic: {self.intent_topic}").grid(row=1, column=0, sticky="w", pady=(4, 0))
        ttk.Checkbutton(header, text="publish", variable=self.publish_enabled).grid(
        ...
        ttk.Button(buttons, text="Forward", command=lambda: self.set_command(OwnerIntent.FORWARD)).grid(
            row=0, column=1, sticky="ew", padx=4, pady=4
        )
        ttk.Button(buttons, text="Left", command=lambda: self.set_command(OwnerIntent.LEFT)).grid(
            row=1, column=0, sticky="ew", padx=4, pady=4
        )
        ttk.Button(buttons, text="Stop", command=lambda: self.set_command(OwnerIntent.STOP)).grid(
            row=1, column=1, sticky="ew", padx=4, pady=4
        )
        ttk.Button(buttons, text="Right", command=lambda: self.set_command(OwnerIntent.RIGHT)).grid(
            row=1, column=2, sticky="ew", padx=4, pady=4
        )
        ttk.Button(buttons, text="Back", command=lambda: self.set_command(OwnerIntent.BACK)).grid(
            row=2, column=1, sticky="ew", padx=4, pady=4
        )
        ttk.Button(buttons, text="Turn L", command=lambda: self.set_command(OwnerIntent.TURN_LEFT)).grid(
            row=0, column=3, sticky="ew", padx=4, pady=4
        )
        ttk.Button(buttons, text="Turn R", command=lambda: self.set_command(OwnerIntent.TURN_RIGHT)).grid(
        ...
        ttk.Label(footer, textvariable=self.last_publish).grid(row=0, column=1, sticky="w", padx=(8, 0))
        ttk.Label(footer, text="cmd_vel:").grid(row=1, column=0, sticky="w", pady=(8, 0))
        ttk.Label(footer, textvariable=self.cmd_vel_status).grid(row=1, column=1, sticky="w", padx=(8, 0), pady=(8, 0))
        ttk.Label(footer, text=f"watching: {self.cmd_vel_topic}").grid(row=2, column=0, columnspan=2, sticky="w", pady=(4, 0))
        ttk.Label(footer, text="feedback:").grid(row=3, column=0, sticky="w", pady=(8, 0))
        ttk.Label(footer, textvariable=self.nav_feedback_status).grid(row=3, column=1, sticky="w", padx=(8, 0), pady=(8, 0))
        ttk.Label(footer, text="decision:").grid(row=4, column=0, sticky="w", pady=(8, 0))
        ttk.Label(footer, textvariable=self.nav_decision_status).grid(row=4, column=1, sticky="w", padx=(8, 0), pady=(8, 0))
    ...
    def on_nav_feedback(self, msg):
        self.nav_feedback_status.set("{}  {}".format(self.feedback_name(msg.status), msg.message))
